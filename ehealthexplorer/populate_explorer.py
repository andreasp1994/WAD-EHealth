...
def populate():
    ...
    jim = User.objects.get(name='jim')
    jill = User.objects.get(name='jill')
    joe = User.objects.get(name='joe')
...
def add_cat(user,name):
    c = Category.objects.get_or_create(user=user)
    c.name = name
    c.save()
    return c
